...
FastAPI と uvicorn で共通ログ検索 API を提供する Python コンテナです。Elasticsearch 公式クライアントを使用します。

## ファイル構成

```text
backend-python/
├── Dockerfile               # Python 依存関係を導入し uvicorn を起動
├── Readme.md                # このファイル
├── app.py                   # FastAPI のルート、入力処理、JSON レスポンス
...
コンテナは内部ポート `5000` で待ち受けます。frontend 側のプロキシパスは
`/api/python/...` と `/health/python` です。
