...
既存の Elasticsearch に保存したログを PHP / Slim のWebアプリからキーワード検索するアプリです。
...
PHP / Slim アプリだけを Docker で起動します。Elasticsearch / Kibana はこの Compose には含めません。
...
PHP の構文チェックで主要ファイルの基本的な妥当性を確認できます。

実行方法:

```bash
php -l src/config.php
php -l src/elasticsearch.php
php -l src/http.php
```
...
本サイトは PHP / Slim 版です。
ブランチを切り替えれば Go / Java / Ruby / Python 版にもなります。
