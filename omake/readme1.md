...
本サイト（`flask_elastic`）の Nginx アクセスログをアクセスログAPIから取得し、
Elasticsearch のデータストリーム `logs-access-elastic` へ日次で取り込む手順です。
既存のアクセスログ取り込み環境と衝突しないよう、シェル、ILMポリシー、
インデックステンプレート、cron設定にも `elastic` 専用名を使用します。

本サイトは標準では次のURLで公開されます。

- サイト: `http://<サイトホスト>:8080`
- アクセスログAPI: `http://<サイトホスト>:8080/api/access-logs`
...
- 実行ホストから本サイトのポート `8080` と Elasticsearchのポート `9200` へ接続できること
...
```bash
curl -fsS "http://<サイトホスト>:8080/api/access-logs?tail=1" | jq .
```
...
```bash
sudo tee /opt/elastic/bin/setup_accesslog_elastic_datastream.sh >/dev/null <<'EOF'
#!/usr/bin/env bash
set -euo pipefail

ES_URL="${ES_URL:-http://elastic1:9200}"
DATA_STREAM="${DATA_STREAM:-logs-access-elastic}"

curl -fsS -X PUT "${ES_URL}/_ilm/policy/logs-access-elastic-14d-policy" \
...
curl -fsS -X PUT "${ES_URL}/_index_template/logs_access_elastic_template" \
  -H "Content-Type: application/json" \
  -d "{
    ...
    \"template\": {
      \"settings\": {
        \"index.lifecycle.name\": \"logs-access-elastic-14d-policy\"
      },
      \"mappings\": {
        \"properties\": {
          \"@timestamp\": { \"type\": \"date\" },
...
sudo chmod 755 /opt/elastic/bin/setup_accesslog_elastic_datastream.sh
```
...
sudo ES_URL="http://elastic1:9200" \
  /opt/elastic/bin/setup_accesslog_elastic_datastream.sh
```
...
```bash
sudo tee /opt/elastic/bin/import_accesslog_elastic_to_es.sh >/dev/null <<'EOF'
#!/usr/bin/env bash
set -euo pipefail

ES_URL="${ES_URL:-http://elastic1:9200}"
ACCESS_LOG_API_URL="${ACCESS_LOG_API_URL:-http://127.0.0.1:8080/api/access-logs}"
DATA_STREAM="${DATA_STREAM:-logs-access-elastic}"
WORK_DIR="${WORK_DIR:-/tmp/accesslog-elastic-to-es}"
...
TMP_JSON="${WORK_DIR}/accesslog_elastic_${TARGET_DT}.json"
TMP_JSONL="${WORK_DIR}/accesslog_elastic_${TARGET_DT}.jsonl"
TMP_BULK="${WORK_DIR}/accesslog_elastic_${TARGET_DT}.bulk"
...
sudo chmod 755 /opt/elastic/bin/import_accesslog_elastic_to_es.sh
```
...
```bash
sudo ACCESS_LOG_API_URL="http://<サイトホスト>:8080/api/access-logs" \
  ES_URL="http://elastic1:9200" \
  /opt/elastic/bin/import_accesslog_elastic_to_es.sh 2026-06-23
```
...
sudo ACCESS_LOG_API_URL="http://<サイトホスト>:8080/api/access-logs" \
  ES_URL="http://elastic1:9200" \
  /opt/elastic/bin/import_accesslog_elastic_to_es.sh
```

取り込み結果の確認:

```bash
curl -fsS "http://elastic1:9200/logs-access-elastic/_search?size=1&sort=%40timestamp%3Adesc" | jq .
```

## cronによる定期実行

接続先を `/etc/default/accesslog-elastic-to-es` に保存します。

```bash
sudo tee /etc/default/accesslog-elastic-to-es >/dev/null <<'EOF'
ACCESS_LOG_API_URL=http://<サイトホスト>:8080/api/access-logs
ES_URL=http://elastic1:9200
DATA_STREAM=logs-access-elastic
EOF
```
...
```bash
sudo tee /etc/cron.d/accesslog_elastic_to_es >/dev/null <<'EOF'
SHELL=/bin/bash
PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin

5 0 * * * root set -a; . /etc/default/accesslog-elastic-to-es; set +a; /opt/elastic/bin/import_accesslog_elastic_to_es.sh >> /var/log/import_accesslog_elastic_to_es.log 2>&1
EOF

sudo chmod 644 /etc/cron.d/accesslog_elastic_to_es
```
